local CoverTile = require("simple_opds/ui/tile")
local Device = require("device")
local Geom = require("ui/geometry")
local HorizontalGroup = require("ui/widget/horizontalgroup")
local HorizontalSpan = require("ui/widget/horizontalspan")
local InputContainer = require("ui/widget/container/inputcontainer")
local Size = require("ui/size")
local VerticalGroup = require("ui/widget/verticalgroup")
local VerticalSpan = require("ui/widget/verticalspan")

local Screen = Device.screen

local CoverGrid = InputContainer:extend{
    width = nil,
    height = nil,
    items = nil,             -- normalized FeedClient items
    cover_cache = nil,
    fetch_cover = nil,       -- function(url, cb)  — async
    on_select = nil,         -- function(item)
    show_parent = nil,
}

local function grid_metrics(width, height)
    local is_landscape = width > height
    local columns = is_landscape and 4 or 3
    local outer_padding = Size.padding.large
    local gap = Size.padding.default
    local tile_w = math.floor((width - 2 * outer_padding - (columns - 1) * gap) / columns)
    local tile_h = math.floor(tile_w * 1.55)
    local tile_total_h = tile_h + 2 * Size.padding.small
    local usable_h = height - 2 * outer_padding
    local max_rows = math.max(1, math.floor((usable_h + gap) / (tile_total_h + gap)))
    return {
        columns = columns, outer_padding = outer_padding, gap = gap,
        tile_w = tile_w, tile_h = tile_h, max_rows = max_rows,
    }
end

function CoverGrid.items_per_page(width, height)
    local m = grid_metrics(width, height)
    return m.max_rows * m.columns
end

function CoverGrid:init()
    self.width = self.width or Screen:getWidth()
    self.height = self.height or Screen:getHeight()

    local m = grid_metrics(self.width, self.height)
    local columns, outer_padding, gap = m.columns, m.outer_padding, m.gap
    local tile_w, tile_h = m.tile_w, m.tile_h
    local max_items = m.max_rows * columns

    local rows = {}
    local items = self.items or {}
    if #items > max_items then
        local trimmed = {}
        for i = 1, max_items do trimmed[i] = items[i] end
        items = trimmed
    end

    -- Track tiles so async cover loaders can refresh them in-place.
    self._tiles = {}
    self._tiles_by_url = {}

    local row, tiles_in_row = {}, 0
    for idx, item in ipairs(items) do
        local cached = item.cover_url and self.cover_cache and self.cover_cache.has(item.cover_url)
        local cover_path = cached and self.cover_cache.path_for(item.cover_url) or nil

        local tile = CoverTile:new{
            item = item,
            cover_path = cover_path,
            width = tile_w,
            height = tile_h,
            show_parent = self.show_parent,
            on_tap = function(it) if self.on_select then self.on_select(it) end end,
        }
        self._tiles[idx] = tile
        if item.cover_url then self._tiles_by_url[item.cover_url] = tile end

        if tiles_in_row > 0 then
            table.insert(row, HorizontalSpan:new{ width = gap })
        end
        table.insert(row, tile)
        tiles_in_row = tiles_in_row + 1

        if tiles_in_row >= columns or idx == #items then
            table.insert(rows, HorizontalGroup:new(row))
            row, tiles_in_row = {}, 0
            if idx ~= #items then
                table.insert(rows, VerticalSpan:new{ width = gap })
            end
        end
    end

    local content = VerticalGroup:new{
        align = "left",
        VerticalSpan:new{ width = outer_padding },
    }
    for _, r in ipairs(rows) do
        table.insert(content, HorizontalGroup:new{
            HorizontalSpan:new{ width = outer_padding },
            r,
        })
    end
    table.insert(content, VerticalSpan:new{ width = outer_padding })

    self[1] = content
    self.dimen = Geom:new{ x = 0, y = 0, w = self.width, h = self.height }

    -- The Shell drives cover fetches in subprocesses + a poller, so we just
    -- expose which URLs need work and a hook for the Shell to refresh tiles.
    if self.fetch_cover then
        for _, item in ipairs(items) do
            if item.cover_url and not (self.cover_cache and self.cover_cache.has(item.cover_url)) then
                self.fetch_cover(item)
            end
        end
    end
end

function CoverGrid:set_cover(url, path)
    local tile = self._tiles_by_url and self._tiles_by_url[url]
    if tile then tile:set_cover_path(path) end
end

return CoverGrid
